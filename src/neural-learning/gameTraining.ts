import {openSync, closeSync, readSync, writeSync, existsSync, mkdirSync, readdirSync} from 'fs'
...
class NeuralNetwork
{
	...
	activationType: number = ActivationType.Sigmoid
	...
	sigmoidActivation(value: number): number
	{
	...
	reluActivation(value: number): number
	{
	...
	defineActivation(activationType: number): void
	{
		...
		{
			case ActivationType.Sigmoid: this.activation = this.sigmoidActivation
										 break
			case ActivationType.Relu   : this.activation = this.reluActivation
										 break
		}
	}
	...
	constructor(layerSizes: number[] = [], activationType = ActivationType.Sigmoid, initialWeightsRange: number[] = [])
	{
		...
		for (let layerIndex = 1; layerIndex < this.layerSizes.length; layerIndex++)
		{
			...
			const currentLayerWeights = new Array<number>((this.layerSizes[layerIndex - 1] + 1) * this.layerSizes[layerIndex])
	...
	loadFromFile(fileName: string): void
	{
		...
		const fileHandle = openSync(fileName, 'r')
		...
		readSync(fileHandle, buffer)
		//Создать список размеров слоев
		this.layerSizes = new Array<number>(buffer.readUint16BE(0))
		...
		for (let layerWeightsIndex = 0; layerWeightsIndex < this.layerSizes.length - 1; layerWeightsIndex++)
		{
			...
			//Создать веса текущего слоя
			const layerWeights = new Array<number>(buffer.readUint16BE(0))
	...
	predict(input: number[]): number[]
	{
		...
		let previousLayerOutput: number[] = []
		...
		previousLayerOutput = new Array<number>(input.length)
		...
		for (let layerWeightsIndex = 0; layerWeightsIndex < this.layerWeights.length; layerWeightsIndex++)
		{
			...
			currentLayerOutput = new Array<number>(this.layerSizes[layerWeightsIndex + 1])
	...
	getLayerSizes(): number[]
	{
		...
		const layerSizes = new Array<number>(this.layerSizes.length)
	...
	getLayerWeights(layerIndex: number): number[]
	{
		...
		const layerWeights = new Array<number>(this.layerWeights[layerIndex - 1].length)
	...
	setLayerWeights(layerIndex: number, weights: number[]): void
	{
		//Пройти по весам заданного слоя
...
class PopulationItem
{
	...
	//Качество (насколько хороша нейронная сеть)
	quality: number
}
...
class TrainingParams
{
	...
	//Количество игр на элемент популяции
	gamesPerPopulationItem: number

	//Размеры слоев нейронных сетей
	layerSizes: number[]
	//Тип активации нейронов
	activationType: number
	//Диапазон начальных весов
	initialWeightsRange: number[]

	//Фактор отбора элементов популяции при эволюции. Должен быть в диапазоне [0; 1]
	selectionFactor: number
	//Фактор мутации. Должен быть в диапазоне [0; 1]
	mutationFactor: number
	//Сила мутации
	mutationStrength: number

	//Путь к каталогу сохранения популяции (без слеша в конце)
	savingPath: string
	//Интервал между сохранениями популяции
	savingInterval: number

	//Флаг вывода на консоль процесса обучения
	verbose: boolean
}
...
function formDataset(datasetItems: DatasetItem[], gamesCount: number, verbose: boolean)
{
		...
		do
		{
			...
			if (listOrFinish instanceof Array)
			{
				...
				for (let positionIndex = 0; positionIndex < listOrFinish.length; positionIndex++)
				{
					...
					const normalizedPosition = new Array<number>(listOrFinish[positionIndex].length - 2)
...
function loadDataset(fileName: string): DatasetItem[]
{
	...
	if (existsSync(fileName))
	{
		...
		const datasetFileHandle = openSync(fileName, 'r')
		...
		while (1)
		{
			...
			const bytesRead = readSync(datasetFileHandle, buffer)
		...
		closeSync(datasetFileHandle)

		//Преобразовать строку элементов набора данных в список элементов набора данных
		datasetItems = <DatasetItem[]> JSON.parse(datasetItemsString)
	}
...
function saveDataset(datasetItems: DatasetItem[], fileName: string): void
{
	//Открыть файл набора данных для записи
	const datasetFileHandle = openSync(fileName, 'w')
	//Записать в файл набора данных элементы набора данных
	writeSync(datasetFileHandle, JSON.stringify(datasetItems))
	//Закрыть файл набора данных
	closeSync(datasetFileHandle)
}
...
//-----------------------------------------------------------------------------
//Создать популяцию
function createPopulation(size: number, networkLayerSizes: number[], activationType: ActivationType, initialWeightsRange: number[]): PopulationItem[]
{
	//Создать популяцию нейронных сетей
	const population = new Array<PopulationItem>(size)
...
//-----------------------------------------------------------------------------
//Загрузить популяцию
function loadPopulation(path: string, trainingInfoFileName: string): {population: PopulationItem[], passedGenerationsCount: number}
{
	//Создать пустую популяцию нейронных сетей
	let population: PopulationItem[] = []
	...
	if (existsSync(path))
	{
		...
		for (let i = 0; i < fileNames.length; i++)
		{
			...
			if (fileNames[i] === trainingInfoFileName)
			{
				...
				const trainingInfoFileHandle = openSync(path + '/' + trainingInfoFileName, 'r')

				//Создать буфер для чтения
				let buffer = Buffer.alloc(30)
				//Прочитать файл информации о тренировке
				const bytesRead = readSync(trainingInfoFileHandle, buffer)
				//Уменьшить размер буфера до количества прочитанных байтов
				buffer = buffer.subarray(0, bytesRead)
				//Задать значение количества поколений популяции нейронных сетей
				passedGenerationsCount = (<{generation: number}> JSON.parse(buffer.toString())).generation

				//Закрыть файл информации о тренировке
				closeSync(trainingInfoFileHandle)
			}
			else
			{
...
//-----------------------------------------------------------------------------
//Сохранить популяцию
function savePopulation(generation: number, population: PopulationItem[], path: string, trainingInfoFileName: string): void
{
	//Если путь каталог сохранения популяции не существует
	if (!existsSync(trainingParams.savingPath))
	{
		//Создать каталог сохранения популяции
		mkdirSync(trainingParams.savingPath)
	}

	//Открыть файл информации о тренировке для записи
	const trainingInfoFileHandle = openSync(path + '/' + trainingInfoFileName, 'w')
	//Записать в файл информации о тренировке номер поколения популяции
	writeSync(trainingInfoFileHandle, JSON.stringify({generation}))
	//Закрыть файл информации о тренировке
	closeSync(trainingInfoFileHandle)
...
function estimatePopulationQuality(population: PopulationItem[], gamesPerPopulationItem: number, verbose: boolean): number
{
...
function evolvePopulation(population: PopulationItem[], evolutionParams: EvolutionParams): PopulationItem[]
{
	//Создать эволюционировавшую популяция
	const evolvedPopulation = new Array<PopulationItem>(population.length)
...
function geneticTrain(trainingParams: TrainingParams): NeuralNetwork
{
	...
	let population: PopulationItem[] = []
	//Задать нулевое значение количества поколений популяции нейронных сетей
	let passedGenerationsCount: number = 0
	...
	else
	{
		...
		population = createPopulation(trainingParams.populationSize, trainingParams.layerSizes,
										trainingParams.activationType, trainingParams.initialWeightsRange)
		//Задать нулевое значение количества поколений популяции нейронных сетей
